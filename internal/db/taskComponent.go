...
type SwaggerSpec struct {
	...
	Definitions map[string]Schemadomain `yaml:"definitions"`
}

type Schemadomain struct {
	Type        string                  `yaml:"type"`
	Required    []string                `yaml:"required"`
	Properties  map[string]Schemadomain `yaml:"properties"`
	Items       *Schemadomain           `yaml:"items,omitempty"`
...
func TaskComponentGetByName(name string) *domain.TaskComponent {
...
func TaskComponentInit() error {
	...
	for _, file := range files {
		...
		var taskComponent *domain.TaskComponent
...
func resolveSchemaRef(ref string, definitions map[string]Schemadomain) Schemadomain {
...
func getPropertyType(schema Schemadomain) string {
	if schema.Type != "" {
		return schema.Type
	}
...
func convertToPropertyDef(schema Schemadomain) domain.PropertyDef {
	property := domain.PropertyDef{
		Type:        getPropertyType(schema),
		Required:    schema.Required,
		Properties:  make(map[string]domain.PropertyDef),
	...
	if property.Type == "array" && schema.Items != nil {
		property.Items = &domain.PropertyDef{
...
func convertSchemaToParams(schema Schemadomain) domain.ParameterStructure {
	params := domain.ParameterStructure{
		Required:   schema.Required,
		Properties: make(map[string]domain.PropertyDef),
	}
...
func generateExampleValue(schema Schemadomain) interface{} {
...
func generateRequestBodyExample(schema Schemadomain) string {
...
func processEndpoint(connectionID string, spec *SwaggerSpec, targetEndpoint, targetMethod string) (*domain.TaskComponent, error) {
	...
	for path, pathItem := range spec.Paths {
		if normalizePath(path) == targetEndpoint {
			...
			taskComponent := &domain.TaskComponent{
				Data: domain.TaskComponentData{
					Options: domain.TaskComponentOptions{
			...
			pathParams := domain.ParameterStructure{
				Properties: make(map[string]domain.PropertyDef),
			}
			queryParams := domain.ParameterStructure{
				Properties: make(map[string]domain.PropertyDef),
			}
			...
			for _, param := range operation.Parameters {
				...
				case "path":
					...
					pathParams.Properties[param.Name] = domain.PropertyDef{
						Type:        param.Type,
				...
				case "query":
					...
					queryParams.Properties[param.Name] = domain.PropertyDef{
						Type:        param.Type,
						Description: param.Description,
						Default:     param.Default,
						Enum:        param.Enum,
